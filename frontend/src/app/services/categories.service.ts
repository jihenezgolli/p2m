...
export class CategoriesService {
  ...
  add(data){
    this.http.post('apiUIrl:8000/api/category', data).subscribe(
      res => {
  ...
  getAllCategories(){
    return this.http.get('apiUIrl:8000/api/category');
  }

  getSingleCategories(id){
    return this.http.get('apiUIrl:8000/api/category/'+id+'/edit');
  }

  delete(id, index) {
    this.http.delete('apiUIrl:8000/api/category/'+id).subscribe(
      res => {
  ...
  edit(data, id, index) {
    this.http.put('apiUIrl:8000/api/category/'+id, data).subscribe(
      res => {
